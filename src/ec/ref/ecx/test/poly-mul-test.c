...
int main()
{
	...
	for (tree_size = 1; tree_size < nmax; tree_size++)
	{
		...
		for (iteration = 0; iteration < nmax + 1 - tree_size; iteration++)
		{
			...
			{
				poly_mul(h, h, len, &F[LENF*i], LENF);
				len += LENF - 1;
			}
	...
	for (tree_size = 1; tree_size < nmax; tree_size++)
	{
		...
		for (iteration = 0; iteration < nmax + 1 - tree_size; iteration++)
		{
			...
			for (i = 0; i < tree_size; i++)
			{
				...
				{
					fp2_random(&F[LENF*i + e]);
					fp2_copy(&F[LENF*i + LENF - 1 - e], &F[LENF*i + e]);
				}
				if (LENF & 1)
				{
					fp2_random(&F[LENF*i + (LENF >> 1)]);
				}
			}
			product_tree_selfreciprocal(H, DEG, 0, 0, F, LENF, tree_size, logn);
